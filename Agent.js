...
const deleteBooking = (request, response) => {
  ...
  const getBookDate=(request,response)=>{
    ...
    connection.query('select truckNumber, date, time, from, to,fromSublocation,toSublocation,totalPrice,tbr,status from booking where  "date"::date >= ?::date AND "date"::date <= ?::date',[from,to],(error,results)=>{
        if(error){
            throw error
        }
        response.status(200).json(results)
